...
# X_train_val_norm, X_test, y_train_val, y_test = train_test_split(X, y, test_size=0.1, random_state=42)
# X_train, X_val, y_train, y_val = train_test_split(X_train_val, y_train_val, test_size=0.15, random_state=42)

tf.config.set_visible_devices([], 'GPU')

def fit_nn_houses_regression_mixed(c):
    print(f"C = {c}")
    ...
    model = tf.keras.models.Sequential([
      ...
      tf.keras.layers.Dense(int(np.around(c[0])), activation='relu'),
      tf.keras.layers.Dense(int(np.around(c[1])), activation='relu'),
      tf.keras.layers.Dense(1, activation='relu')
    ])

    model.compile(
        loss=Custom_Loss(c[2]/1000),
